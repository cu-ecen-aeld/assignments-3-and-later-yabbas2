									  ...
									  size_t *entry_offset_byte_rtn)
{
	...
	do {
		...
		if (accum_offset + entry->size > char_offset) {
			*entry_offset_byte_rtn = char_offset - accum_offset;
			return entry;
		}
		accum_offset += entry->size;
...
{
	struct aesd_buffer_entry *circ_buf_entry = &buffer->entry[buffer->in_offs];
	circ_buf_entry->buffptr = add_entry->buffptr;
	circ_buf_entry->size = add_entry->size;
