...
static ssize_t aesd_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
	...
	int ret = mutex_lock_interruptible(&aesd_dev.mutex);
    if (ret != 0) {
        return ret;
    }
	...
	struct aesd_buffer_entry *entry =
		aesd_circular_buffer_find_entry_offset_for_fpos(&aesd_dev.circ_buf, pos, &pos_in_entry);
    if (entry != NULL) {
        size_t bytes_left_in_entry = entry->size - pos_in_entry;
        size_t bytes_to_read = min(bytes_left_in_entry, count);

        ssize_t not_copied = copy_to_user(buf, entry->buffptr + pos_in_entry, bytes_to_read);
...
static ssize_t aesd_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
	...
	ssize_t not_copied = copy_from_user(wrtbuf.buffptr + wrtbuf.size, buf, count);
	if (not_copied == count) {
		return -EFAULT;
	}
	count -= not_copied;
...
static int aesd_dev_uevent(const struct device *dev, struct kobj_uevent_env *env)
{
...
static int __init aesd_init_module(void)
{
	...
	aesd_class = class_create(DRV_NAME);
	if (!aesd_class) {
		status = -ENOMEM;
		goto free_chrdev;
	}
